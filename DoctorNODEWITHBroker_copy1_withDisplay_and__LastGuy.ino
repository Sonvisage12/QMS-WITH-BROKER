...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (uid == "NO_PATIENT") {
    ...
    else if (uid == "SOLENOID") {
   
    //lcd.clear();
    lcd.setCursor(15, 0); lcd.print("G");
...
void sendRequestToQueueManager1() {
  StaticJsonDocument<256> doc;
  doc["uid"] = "REQ_BREAK";
  doc["timestamp"] = millis();
  doc["node"] = nodeID;
...
void reconnectMQTT() { lcd.clear();
  while (!mqttClient.connected()) {
    ...
    String clientId = "DoctorNode-" + String(nodeID);
...
pinMode(BTN_RESET_PIN, INPUT);
  ...
  lcd.clear();
prefs.begin("solenoid", false);
solenoidState = prefs.getBool("state", false);  // default OFF
prefs.end();

lcd.setCursor(15, 1);  // Last column, second row
lcd.print(solenoidState ? "1" : "0");  // Display ON (1) or OFF (0)
...
if (digitalRead(BTN_CLEAR1_PIN) == HIGH && digitalRead(BTN_CLEAR2_PIN) == HIGH) {
  Serial.println("🧹 Remove from queue via button Pressed Together");
...
if (digitalRead(BTN_CLEAR1_PIN) == HIGH ) {
Serial.println("🔘 Request Button Pressed");
 N=10; 
delay(500);
}
  ...
  if (patientReady && scannedUID == String(currentUID)) {
    ...
    patientReady = false;
    if (N==10){
  sendRequestToQueueManager1();blinkLED(GREEN_LED_PIN);
    }
    else{
    sendRequestToQueueManager();
